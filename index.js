...
var CreditCardSchema = object({
  ...
  ).required("Expiration year is required.").typeError("Expiration year must be a number.").min(new Date().getFullYear(), "Expiration year must not be in the past.").max(new Date().getFullYear() + 20, "Expiry too far in the future."),
...
var domainSchemaBase = object4().shape({
  ...
  tags: array3(),
  description: string4().min(1, "Description must be between 1 and 255 characters.").max(255, "Description must be between 1 and 255 characters."),
  retry_sec: number3(),
...
//import { parseCIDR as parseCIDR2 } from "ipaddr.js";
//LDO
import pkg2 from 'ipaddr.js';
const { parseCIDR: parseCIDR2 } = pkg2;
